...
def test_hausdorff_distance():
    ...
    # Result 10.
    s5 = Set([Interval(float('-inf'), 0)])
    s6 = Set([Interval(float('-inf'), 10)])
    assert s5.hausdorff_distance(s6) == 10.0

def test_distance_edge_cases():
    """Test edge cases for Set distance."""
    s1 = Set([Interval(0, 1)])
    empty = Set()
    ...
    # s1=[0,1], s3=[2,3]U[5,6]
    s3 = Set([Interval(2, 3), Interval(5, 6)])
...
def test_hausdorff_infinite_bounds_edge_cases():
    ...
    s_inf_start = Set([Interval(float('-inf'), 0)])
    s_bounded = Set([Interval(0, 10)])
    
    # directed(s_inf_start, s_bounded) -> inf
    assert s_inf_start.hausdorff_distance(s_bounded) == float('inf')
    
    # Source has inf, Target does not
    s_inf_end = Set([Interval(0, float('inf'))])
...
def test_distance_to_point_edge_cases():
    ...
    s_open = Set([Interval(0, 10, open_start=True)])
    assert 0 not in s_open
    assert s_open.distance_to_point(0) == 0.0
    
    # Same for end
    s_open_end = Set([Interval(0, 10, open_end=True)])
...
def test_metrics_final_coverage():
    ...
    s4 = Set([Interval(0, float('inf'))])
    s5 = Set([Interval(10, float('inf'))])
    ...
    s6 = Set([Interval(0, 2), Interval(10, 12)])
