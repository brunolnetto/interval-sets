...
def demo_mixed_geometry():
    section("2. Mixed Dimensionality in a BoxSet")

    # A BoxSet can hold 'Boxes' of different effective dimensionality
    ...
    print(f"BoxSet contains {len(universe.boxes)} disjoint boxes.")
...
def demo_1d_isomorphism():
    section("3. 1D BoxSet vs 1D IntervalSet")

    # Conceptually, BoxSet(dim=1) is the same as IntervalSet.
    # IntervalSet is optimized (O(N log N) merge), BoxSet is generic (recursive slicing).
    ...
    s = IntervalSet([i1, i2])
    r = BoxSet([Box([i1]), Box([i2])])  # 1D boxes
    ...
    r_diff = r.difference(BoxSet([Box([sub])]))

    print(f"IntervalSet Diff: {s_diff}")  # Should be {[0, 1), (5, 6]}
    print(f"BoxSet Diff: {r_diff}")

    # They should represent the sameremaining space
    # BoxSet might strictly output closed boxes if slicing logic defaults to closed?
