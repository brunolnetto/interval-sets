...
def main():
    ...
    print(
        f"  Busy time: {busy_hours:.1f} hours ({busy_hours/total_work_hours*100:.1f}%)"
    )
    print(
        f"  Available time: {available_hours:.1f} hours ({available_hours/total_work_hours*100:.1f}%)"
    )
    ...
    new_meeting = Interval.open(
        10, 11
    )  # (10, 11) AM - open intervals don't include endpoints
